...
import { AlertTriangle, CheckCircle, CreditCard, ExternalLink, Loader2, Mail, MapPin, Package, Receipt, Tag, Truck, X } from "lucide-react"
...
const OrderDetailsModal = ({ orderId, onClose }: OrderDetailsModalProps) => {
     ...
     return createPortal(
          <div className="fixed inset-0 z-[9999] flex justify-end font-sans pointer-events-none">
               ...
               {/* AnimatePresence handles the exit animation when orderId becomes null */}
               <AnimatePresence>
                    {orderId && (
                         <>
                              {/* Backdrop */}
                              ...
                              <motion.div
                                   ...
                                   initial={{ x: "100%" }}
                                   // Slide to center (Right-to-Left visual movement)
                                   animate={{ x: 0 }}
                                   // Slide back off-screen right (Left-to-Right visual movement)
                                   exit={{ x: "100%" }}
                                             ...
                                             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                  ...
                                                  <div className="bg-stone-50 p-5 rounded-2xl border border-stone-100 h-full flex flex-col">
                                                       ...
                                                       {order.shippingAddress ? (
                                                            ...
                                                            <div className="flex-1 flex items-center justify-center text-stone-400 italic text-sm">No shipping address</div>
                                                       )}
                                                  </div>
                                             </div>
                                                       ...
                                                       {order.items?.map((item: any) => (
                                                            <div key={item.id} className="p-4 bg-white flex items-center gap-4">
                                                                 <div className="w-12 h-12 bg-stone-100 rounded-lg shrink-0 overflow-hidden">
                                                                      {item.product?.image && (
                                                                           <img src={item.product.image} className="w-full h-full object-cover" alt={item.productName} />
                                                                      )}
                                                                 </div>
