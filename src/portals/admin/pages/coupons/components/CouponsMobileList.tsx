import { Archive, Calendar, Edit2, RotateCcw, Ticket } from "lucide-react"
...
const CouponsMobileList = ({ coupons, isLoading, onEdit, onArchive }: CouponsMobileListProps) => {
     if (isLoading) {
          return (
     ...
     return (
          <div className="grid grid-cols-1 gap-4 md:hidden pb-20">
               {coupons.map(coupon => (
                    <MobileCouponCard key={coupon.id} coupon={coupon} onEdit={onEdit} onArchive={onArchive} />
               ))}
          </div>
     )
}

const MobileCouponCard = ({ coupon, onEdit, onArchive }: { coupon: ICoupon; onEdit: any; onArchive: any }) => (
     <div
          ...
          <div className="flex gap-3">
               ...
               <button
                    ...
                    className={`flex-1 py-3 bg-white border border-stone-200 font-bold text-xs uppercase tracking-wider rounded-xl transition-colors flex items-center justify-center gap-2 ${
                         coupon.isActive ? "text-red-600 hover:bg-red-50 hover:border-red-100" : "text-emerald-600 hover:bg-emerald-50 hover:border-emerald-100"
                    }`}
               >
                    {coupon.isActive ? (
                         <>
